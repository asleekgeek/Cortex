...
from benchmarks.lib._e2_conditions import apply_condition, heat_for, restore_env  # noqa: E402
...
def _emit_banner() -> None:
    """Print the latency-only banner to stderr at runner startup."""
    print(
        "[latency_runner] Latency-only benchmark; retrieval metrics are not "
        "claim-bearing (see module docstring; use e2_subsample_runner / "
        "e2_zipf_runner for E2 retrieval).",
        file=sys.stderr,
    )
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        description="Latency-only benchmark; retrieval metrics not claim-bearing."
    )
    ...
    print(f"== latency :: n={n_values} queries={args.queries} seed={args.seed} ==")
