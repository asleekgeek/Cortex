...
def _resolve_db_url(name_or_url: str) -> str:
    return (
        name_or_url
        if "://" in name_or_url
        else f"postgresql://localhost:5432/{name_or_url}"
    )
...
def create_snapshot(
    db_url: str, snapshot_path: Path, *, allow_prod: bool = False
) -> SnapshotMeta:
    ...
    meta = SnapshotMeta(
        ...
        sha256=fingerprint(snapshot_path),
        n_memories=n_mem,
        n_entities=n_ent,
...
def _check_version_drift(meta: dict, live: dict) -> list[str]:
    ...
    if (
        snap_sha
        and live_sha
        and snap_sha not in ("absent",)
        and live_sha not in ("absent",)
    ):
...
def verify_compatibility(
    snapshot_path: Path, target_db_url: str
) -> CompatibilityReport:
    ...
    try:
        ...
        return CompatibilityReport(
            False, {"_error": {"snap": "", "live": str(exc), "match": False}}
        )
    fields = {
        f: {
            "snap": meta.get(f),
            "live": live.get(f),
            "match": str(meta.get(f)) == str(live.get(f)),
        }
...
def _diff_settings(snap: dict, live: dict) -> dict:
    return {
        n: (str(sv), str(live.get(n, "")))
        for n, sv in (snap or {}).items()
        if str(sv) != str(live.get(n, ""))
    }
...
def restore_snapshot(
    ...
    if drift:
        return RestoreReport(
            False, 0.0, 0, 0, [f"version_drift: {'; '.join(drift)}"], drift, {}
        )
    t0 = time.monotonic()
    ...
    if res.returncode != 0:
        return RestoreReport(
            False, wall, 0, 0, [f"pg_restore: {res.stderr.strip()}"], drift, {}
        )
    n_mem, n_ent = _count_rows(db_url)
    mismatch: list[str] = []
    ...
    sd = _diff_settings(
        meta.get("pg_settings_relevant") or {},
...
def _cmd_create(args: argparse.Namespace) -> int:
    m = create_snapshot(
        _resolve_db_url(args.db), Path(args.out), allow_prod=args.allow_prod
    )
    ...
    print(
        f"  hnsw: {len(m.hnsw_indexes)} indexes"
        if m.hnsw_indexes
        else "  hnsw: (empty — pre-build)"
    )
    return 0
