...
retrieval prior.  This design is asymptotically broken.  As the
corpus grows, top-$k$ retrieval degenerates into near-arbitrary
tie-breaking among items with comparable surface similarity, and
the discriminative information delivered to the consumer LLM
approaches zero---a failure mode that compounds the position bias
of long-context decoding \citep{Liu2023}.  We formalise this
collapse, then describe Cortex, a memory architecture that
maintains a non-flat priority distribution across $N$ by coupling
four mechanisms: (i)~continuously decaying \emph{heat}
\citep{Ebbinghaus1885}, (ii)~a hierarchical predictive-coding
write gate \citep{Friston2010}, (iii)~consolidation cascades that
...
\emph{not} commensurable with BEAM's end-to-end LLM-as-judge score;
we therefore make no head-to-head BEAM claim and use BEAM only for
within-system, same-harness comparisons (\S\ref{sec:beam-proxy}).
A direct decay dose--response on BEAM, re-measured forensically on a
clean store, reverses an earlier finding of ours: on a freshly loaded
corpus decay contributes no ranking benefit (six-point
$\lambda$-sweep, 395 questions; $\lambda = 1.0$ weakly optimal at MRR
0.545 versus a flat 0.503 plateau for all $\lambda < 1$).  An earlier
two-point sweep that had shown $+68\%$ from decay is traced, by
factor isolation (same code, same sample, clean versus contaminated
store), to store contamination: decay had been cooling stale
out-of-task memories below the retrieval heat floor.  We therefore
re-scope decay's measured value as store \emph{hygiene} and capacity
control---protective in long-lived stores that accumulate stale
content, neutral-to-slightly-negative for ranking on clean
single-pass loads (\S\ref{sec:decay-dose}).  We characterise when
flat memory remains adequate, quantify the calibration cost of
decay, and bound the per-write overhead of biological
consolidation.  Code, data, and benchmark harness are publicly
available.\footnote{\url{https://github.com/cdeust/Cortex}}
\end{abstract}
...
The default external memory for LLM agents is a variant of the same
recipe: embed the input, write to a vector index, and at query time
retrieve the top-$k$ nearest neighbours, optionally re-ranked.  This
recipe succeeds at the document-retrieval scale it was designed for
\citep{Lewis2020}, but it does not scale to what agentic systems
are now demanding of it---open-ended, multi-month, mixed-content
memory across thousands of conversations.  The bottleneck is not
retrieval speed but \emph{retrieval semantics}: a vector store
treats every entry as equally important in the long term, with no
mechanism to weight a fact mentioned six months ago against a
decision anchored last week.  As $N$ grows, the top-$k$ for a
...
bias documented in \emph{Lost in the Middle} \citep{Liu2023}---then
processes those items in an order uncorrelated with their actual
usefulness.

We argue that the missing mechanism is decay, and that decay is not
a heuristic but a structural requirement: it is what keeps the
priority distribution over memories non-flat at any $N$.  This paper
characterises the flat-importance failure mode, describes Cortex's
...
\begin{enumerate}
  \item A formal statement of the \emph{flat-importance failure
        mode} (\S\ref{sec:flat-failure}): an asymptotic argument
        for why uniform retrieval priors collapse top-$k$
        discriminability as $N$ grows, independent of LLM
        consumption.
  \item A \emph{thermodynamic memory architecture}
        (\S\ref{sec:thermodynamic}) coupling Ebbinghaus heat decay,
        a Friston predictive-coding write gate, coupled
        neuromodulation, synaptic/emotional tagging, and a
        Kandel/McClelland consolidation cascade---each module
        traceable to a primary source and a Cortex implementation
        file.
  \item A \emph{three-form argument} (\S\ref{sec:why-decay}) that
...
\S\ref{sec:related} positions the work against prior systems;
\S\ref{sec:flat-failure} formalises the failure mode;
\S\ref{sec:thermodynamic} describes the architecture;
\S\ref{sec:why-decay} states the load-bearing argument;
\S\ref{sec:empirical} reports benchmarks;
\S\ref{sec:discussion} documents limitations.  Appendices give
Shannon, Erd\H{o}s, Popper, and Feynman treatments of the same
claims at progressively shorter length.
...
Our heat mechanism is closely related to ACT-R base-level activation
\citep{Anderson1991}, in which a memory's retrievability is a
power-law-decaying sum over its past access events, and to the
importance $\times$ recency $\times$ similarity retrieval rule of
Generative Agents \citep{Park2023}.  The Ebbinghaus exponential we
use (\S\ref{sec:heat}) is the same family of forgetting curve, and
the ACT-R lifetime computation that Cortex routes through
\texttt{ingested\_at} (\S\ref{sec:cadence-fix}) is the discrete-event
analogue.  We do not claim a new activation function; we claim its
integration into a five-signal fusion retriever (\S\ref{sec:wrrf}) and
the characterisation of when it helps (\S\ref{sec:regime}).
...
None of the prior work establishes when flat-importance retrieval
\emph{must} fail.  We supply that argument next.
...
density of retrieved tokens has dropped in proportion.  The next
section describes the architecture by which Cortex preserves a
non-flat priority distribution and so avoids this regime.
...
valence and confidence.  Writing $\lambda_v$ for the
valence-modulated factor and applying the confidence modifier as a
second nested resistance term:
\begin{align}
\lambda_{v} &= 1 - \frac{1 - \lambda_{\mathrm{base}}}
                        {1 + |v| \cdot r \cdot (1 - e^{-t})},
\label{eq:lambda-v}\\[2pt]
\lambda_{\mathrm{eff}} &= 1 - \frac{1 - \lambda_{v}}
                                   {1 + \kappa \cdot \mathrm{conf}}
\label{eq:lambda-eff}
\end{align}
...
advantage grows with delay \citep{Yonelinas2015}.  The confidence
modifier ($\kappa = 0.1$, $\mathrm{conf} \in [0,1]$) is an
engineering term with no paper source; it further slows decay for
high-confidence memories.  Half-lives at the default
$\mathrm{conf} = 1$ (with $v = 0$): $\approx 15$\,h for normal
items, $\approx 381$\,h for important items; the
confidence-independent form ($\mathrm{conf} = 0$) gives
$\approx 14$\,h and $\approx 346$\,h.  The constants are calibrated
to a hours/days timescale and are not from a single paper; they are
documented as engineering choices in the source.
...
(``Hebb's curve'') is due to \citet{Hebb1955}; it is often
attributed to \citet{Yerkes1908}, but that study measured stimulus
strength against habit-formation rather than arousal against
performance.  \citet{Qasim2023} show that
high-frequency activity in the amygdala and hippocampus increases
during successful encoding of emotionally arousing stimuli, a
neural correlate of the brain prioritising emotionally salient
inputs for storage.
...
Together these mechanisms instantiate a non-flat priority field over
the corpus.  We now explain why that field is what prevents
asymptotic collapse.
...
\subsection{Operational---WSF Fusion}
\label{sec:wrrf}
...
\emph{Weighted Score Fusion} (WSF) with theoretical-min-max (TMM)
normalisation \citep{Bruch2023} over five independent signals:
\begin{equation}
\mathrm{score}(x \mid q) \;=\;
\sum_{c \in \{\mathrm{vec}, \mathrm{fts}, \mathrm{tri}, \mathrm{heat}, \mathrm{rec}\}}
w_c \cdot
\frac{s_c(x \mid q) - m_c}{\max_{x'} s_c(x' \mid q) - m_c}
\label{eq:wrrf}
\end{equation}
where $s_c$ is the raw per-signal score, $m_c$ its theoretical
minimum ($m_{\mathrm{vec}} = -1$ for cosine similarity, $m_c = 0$ for
the others), and the denominator is the observed per-query maximum
(TMM normalisation, \citealt{Bruch2023}).  Weights $w_c$ are
intent-conditioned (\texttt{mcp\_server/core/pg\_recall.py},
\texttt{compute\_pg\_weights}); the recency channel carries weight
$0$ except for temporal, knowledge-update, and event intents, so the
default active set is four signals.  The trigram signal (\texttt{tri})
is the Postgres \texttt{similarity()} score; there is no separate
n-gram channel.  When two items have equivalent vector and FTS
scores, the heat and recency channels resolve the tie---so the
priority field $P(x)$ acts not just as a prior but as a deterministic
disambiguator in the regime where similarity is uninformative.  This
is the operational mechanism that prevents
\S\ref{sec:flat-collapse}'s tie-noise.  (Reciprocal-rank fusion
\citep{Cormack2009} was the design's starting point and gives the
system its historical name; the deployed function is the score-fusion
variant above, following the score-fusion family analysed by
\citet{Bruch2023}, who report that a convex combination of scores
outperforms RRF in both in-domain and out-of-domain settings and that
RRF is sensitive to its parameters. A controlled within-Cortex
ablation reproduces this direction on our own workload: swapping
\emph{only} the fusion function over identical
per-signal ranked lists on LoCoMo (all 10 conversations, 1{,}982
questions; MiniLM vector plus lexical signals, reranker disabled to
isolate the fusion step), score fusion improves mean reciprocal rank
over reciprocal-rank fusion by ${+}0.11$ (RRF $0.65 \to$ score fusion
$0.76$) while recall@10 is unchanged (${+}0.0005$); score fusion wins
MRR in every question category. The gain is concentrated in ranking
quality rather than recall, consistent with score fusion's use of
calibrated signal magnitudes where reciprocal-rank fusion sees only
ranks. This in-process ablation uses the same signals but not the
production PostgreSQL path or cross-encoder rerank, so its absolute
numbers are not directly comparable to the headline results of
\S\ref{sec:empirical}; only the RRF-vs-score-fusion contrast is
intended. See \texttt{benchmarks/locomo/rrf\_vs\_tmm\_ablation.py}.)
...
absent, the WSF tie-breaker simply chooses the most-prior of a set
...
We turn next to the question this argument forces: do the
mechanisms predicted to preserve discriminability actually do so on
benchmarks designed to defeat flat retrieval?
...
\vspace{2pt}
{\footnotesize $^\dagger$BEAM's published metric is end-to-end
LLM-as-judge nugget scoring; our MRR measures whether the gold
memory is retrieved.  The two are not commensurable, so no
paper-best column is shown for BEAM.  LongMemEval and LoCoMo rows
are retrieval recall, directly comparable to their published
retrieval baselines.}
\end{table}
...
end-to-end metric.  Both benchmarks include question categories
...
(iii)~the BEAM number is a retrieval-proxy MRR
(\S\ref{sec:beam-proxy}), not BEAM's end-to-end metric---see
...
\subsection{Direct Dose--Response: Decay On vs.\ Off on BEAM
(a Negative Result, and the Confound That Hid It)}
\label{sec:decay-dose}

The per-mechanism ablations of \S\ref{sec:per-mechanism} are run on
isolated-haystack benchmarks where every memory is written
milliseconds before retrieval, so the heat distribution is uniform
by construction (\S\ref{sec:regime}).  To measure decay where its
mechanism-of-action could apply, we run a direct dose--response on
BEAM-100K, varying only the per-hour heat-retention factor
$\lambda_{\mathrm{base}}$, holding the entire rest of the stack and
the same \texttt{recall\_memories()} code path fixed.

\paragraph{Clean-store dose--response (the result).}
Six-point $\lambda$-sweep on a freshly created benchmark database,
full 395-question BEAM-100K.  Artefact:
\texttt{benchmarks/results/decay\_sweep/20260611T052523Z/}.

\begin{center}
\small
\begin{tabular}{lrr}
\toprule
$\lambda_{\mathrm{base}}$ & retrieval-proxy MRR & R@10 \\
\midrule
$0.90$  & 0.503 & 0.687 \\
$0.925$ & 0.500 & 0.684 \\
$0.95$ (engineering default) & 0.503 & 0.684 \\
$0.975$ & 0.503 & 0.687 \\
$0.99$  & 0.503 & 0.684 \\
$1.00$ (decay off) & \textbf{0.545} & \textbf{0.744} \\
\bottomrule
\end{tabular}
\end{center}

Decay-off is weakly optimal ($+0.042$ MRR over the flat
$\approx 0.503$ plateau spanning every $\lambda < 1$;
\texttt{analysis.json}: \texttt{optimum\_lambda} $= 1.0$).  On a
clean store, decay contributes no ranking benefit on BEAM and costs
a small amount.  This is a negative result for the hypothesis that
heat decay is a retrieval-\emph{quality} mechanism in this setting,
and it corrects an earlier finding of ours, as follows.

\paragraph{The earlier artefact and its forensic resolution.}
An earlier two-point sweep
(\texttt{benchmarks/results/decay\_sweep/20260430T111134Z/}, quick
mode $=$ 3 BEAM conversations) had shown the opposite: $\lambda =
0.95$ at MRR 0.671 versus $\lambda = 1.0$ at 0.399, a $+0.272$
($+68\%$) advantage for decay that earlier drafts of this paper
reported as load-bearing evidence.  When the clean six-point sweep
above failed to reproduce it, we isolated the factors one at a time:

\begin{center}
\small
\begin{tabular}{llrrr}
\toprule
code path & store & $\lambda{=}0.95$ & $\lambda{=}1.0$ & $\Delta$ \\
\midrule
Apr-30 commit & contaminated & 0.671 & 0.399 & $+0.272$ \\
Apr-30 commit & clean        & 0.671 & 0.671 & $0.000$ \\
current       & clean        & 0.694 & 0.694 & $0.000$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent (All three rows: identical quick-mode 3-conversation
sample.  Row 1 is the committed artefact; rows 2--3:
\texttt{benchmarks/results/decay\_sweep/20260611T081541Z-apr30code-cleandb/}
and \texttt{.../20260611T080828Z/}.)  Re-running the \emph{exact}
April commit on a freshly created database lifts the decay-off
condition from 0.399 to 0.671 and erases the entire gap.  The only
factor that changes between rows 1 and 2 is database cleanliness:
the $+68\%$ was a contaminated-store confound, not a property of the
decay mechanism.

\paragraph{What the confound reveals (decay as hygiene).}
The explanation consistent with all four measurements: in the
contaminated store, stale out-of-task memories---rows predating or
escaping the harness's per-run benchmark purge, which removes only
rows flagged \texttt{is\_benchmark}---competed with the gold
memories at retrieval time.  With decay
enabled, those stale items had cooled below the recall heat floor
(\texttt{recall\_memories()} filters candidates at
\texttt{effective\_heat} $\geq$ \texttt{p\_min\_heat}) and were
excluded; with $\lambda = 1.0$ they stayed permanently hot and
polluted the ranking.  Decay was not improving the ranking of
relevant memories---it was suppressing garbage.  On a clean store
there is no garbage to suppress, and because BEAM ingests every
memory at effectively one instant, \texttt{effective\_heat} is
near-uniform across items and the ranking is invariant to $\lambda$
(rows 2--3: bit-identical MRR at both $\lambda$ values).  We could
not re-inspect the original contaminated store (it was rebuilt
between runs), so the cooling-below-floor mechanism is the inferred
explanation consistent with the data rather than a directly observed
one; the factor isolation itself (contamination, not code, produced
the gap) is directly measured.

\paragraph{Re-scoped claim.}
We withdraw the claim that decay is causal for retrieval quality on
heterogeneous-access corpora.  The defensible, measured claim is
operational: decay bounds the effective working set of a long-lived
store and suppresses accumulated stale content---a \emph{hygiene and
capacity} mechanism.  The contaminated-store row is, read honestly,
a demonstration of exactly that value ($0.399 \to 0.671$ proxy-MRR
recovery in a polluted store), and the longitudinal aging control
(\S\ref{sec:regime}) shows the complementary safety property: decay
does not erode old-but-relevant items within a year.  Whether decay
earns retrieval-quality lift in stores with genuinely differential
\emph{access} histories accumulated over weeks of live use (rather
than single-instant benchmark ingestion) remains an open question
that none of our current benchmarks can answer
...
\paragraph{Statistical caveat (read first).}  All per-mechanism
deltas below are single seed; the empirical per-row noise floor is
$\approx \pm 0.001$ MRR (LME-S, $n{=}500$) and $\approx \pm 0.002$
MRR (LoCoMo, $n{=}1986$).  Deltas below this floor are not
interpretable as causal contributions.  In the tables below,
above-floor deltas are set in \textbf{bold} and sub-floor deltas in
\emph{italic} (bold $=$ above noise floor); sub-floor magnitudes
such as $\pm0.0001$ should be read as $\approx 0$ (within noise).
...
the pre-registration brief in \texttt{docs/benchmarks/e1-v3-results.md}.
...
\emph{HOPFIELD}              & 0.9117 & 0.980 & $\approx 0$ & +0.004 \\
\emph{HDC}                   & 0.9125 & 0.982 & $\approx 0$ & +0.002 \\
\emph{SPREADING\_ACTIVATION} & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{DENDRITIC\_CLUSTERS}   & 0.9126 & 0.984 & $\approx 0$ & 0      \\
EMOTIONAL\_RETRIEVAL  & 0.9134 & 0.984 & \textbf{$-$0.0010} & 0      \\
ADAPTIVE\_DECAY       & 0.9138 & 0.984 & \textbf{$-$0.0014} & 0      \\
\emph{CO\_ACTIVATION}        & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{SURPRISE\_MOMENTUM}    & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{OSCILLATORY\_CLOCK}    & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{PREDICTIVE\_CODING}    & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{NEUROMODULATION}       & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{PATTERN\_SEPARATION}   & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{EMOTIONAL\_TAGGING}    & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{SYNAPTIC\_TAGGING}     & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{ENGRAM\_ALLOCATION}    & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\emph{RECONSOLIDATION}       & 0.9124 & 0.984 & $\approx 0$ & 0      \\
\bottomrule
\end{tabular}\\[2pt]
{\footnotesize Bold $=$ above noise floor ($|\Delta\text{MRR}| >
0.001$); italic rows and ``$\approx 0$'' entries are within the
$\pm 0.001$ MRR noise floor.}
\end{center}
...
\paragraph{Reconciling the isolated-haystack ablation.}
The per-mechanism ablations of this subsection are run under the
LME-S / LoCoMo \texttt{db.clear()} $\to$ \texttt{load} $\to$
\texttt{recall} harness, in which every memory is written
milliseconds before retrieval.  On such a corpus the heat
distribution is uniform by construction, so a forgetting curve has
no differential signal to contribute and can only mis-rank
uniformly-fresh items---hence the small negative $\Delta$ (LME-S
$-0.0014$, LoCoMo $-0.0163$).  This is not evidence against decay;
it is the predicted null (and slight harm) of applying a
longitudinal mechanism to a corpus with no longitudinal structure.
The decay dose--response of \S\ref{sec:decay-dose}, run on BEAM on a
clean store, agrees: decay is neutral-to-slightly-negative for
ranking there too ($0.503$ vs $0.545$ at $\lambda{=}1.0$, full
395-question sample), because benchmark ingestion writes every
memory at effectively one instant and leaves no differential heat
for decay to exploit.  The two results are consistent in the
opposite direction from our earlier draft: across every benchmark we
can measure, decay adds no ranking signal; its measured value
appears only when the store is contaminated with stale content,
where it acts as hygiene (\S\ref{sec:decay-dose}).
...
\texttt{docs/benchmarks/e1-v3-per-category.md}):
...
\item \emph{Read-path rerank stages} (HOPFIELD, HDC,
  SPREADING\_ACTIVATION, DENDRITIC\_CLUSTERS).  The WSF baseline
...
\subsubsection{LoCoMo evidence: empirical resolution of the architectural-mismatch hypothesis}
\label{sec:locomo-evidence}
\label{sec:locomo-forthcoming}

The LME-S analysis above identified three classes of mechanism whose
mechanism-of-action is foreclosed by the LME-S
\texttt{db.clear() $\to$ db.load(haystack) $\to$ db.recall(query)}
per-question architecture: read-path rerank in a saturated regime,
affect-side stages on factual queries, and longitudinal mechanisms
whose state is wiped per question.  We claimed the longitudinal class
would show up on a longitudinal benchmark.  We now measure that.

The LoCoMo ablation is a 14-row, two-baseline, single-seed sweep on
the full benchmark ($n = 1986$).  The two-baseline structure is
necessary because a single anchor cannot fairly evaluate both
classes: mechanisms whose mechanism-of-action requires session
continuity at recall time are ablated against
\texttt{BASELINE\_NO\_CONSOLIDATION} (consolidation off), while
mechanisms that fire only during consolidation are ablated against
\texttt{BASELINE\_WITH\_CONSOLIDATION} (consolidation on).  Each
anchor reflects the active mechanism set at recall time for that
group's mechanism-of-action.  Sign convention is unchanged from
\S\ref{sec:per-mech-table}: $\Delta\text{MRR} = \text{anchor} -
\text{ablated}$.
...
\end{tabular}\\[2pt]
{\footnotesize Bold $=$ above noise floor ($|\Delta\text{MRR}| >
0.002$ at $n{=}1986$); italic rows and ``$\approx 0$'' entries are
within the $\pm 0.002$ MRR noise floor.}
\end{center}

\paragraph{Empirical resolution of the LME-S architectural-mismatch
hypothesis.}  The three longitudinal mechanisms LME-S could not
exercise show up on LoCoMo with consistent signs and magnitudes:
...
This is the load-bearing finding of \S\ref{sec:locomo-evidence}.  The
\S\ref{sec:per-mech-architectural} argument (that 13 LME-S rows were
\emph{predicted-null by construction}, not failed mechanisms) is now
empirically substantiated for the longitudinal subset: when the
benchmark exercises the mechanism-of-action, the mechanism shows up
in the deltas.
...
scale---the same calibrated-stack property
...
the six post-WSF rerank-blend constants.  These constants were
swept under a pre-registered protocol
(\texttt{docs/provenance/blend-weight-calibration.md}):
...
cite \texttt{docs/provenance/blend-weight-calibration.md} as confirmed
near-optimum.  The 17-row ablation table above is therefore
measured at a calibrated equilibrium, not at an arbitrary set of
placeholders.

\paragraph{Verification surfaced two production fixes.}
The same verification campaign surfaced two production-relevant bugs,
both now fixed: a consolidation-cadence bug that read wall-clock
\texttt{created\_at} instead of elapsed-since-ingest (commit
\texttt{6c51bce}, introducing \texttt{memories.ingested\_at}), and a
...
\item \emph{Single-seed per run.}  Each row in
  ...
  interpretable as causal contributions.  The paper-bearing claims
  of \S\ref{sec:per-mechanism} are the
...
\item \emph{Two benchmarks, complementary architectures.}  LME-S
  (\S\ref{sec:per-mech-table}) captures saturated-rerank and
  integrated-stack behaviour; LoCoMo (\S\ref{sec:locomo-evidence})
  captures longitudinal mechanism behaviour.  The two together cover
  the read-path, write-path, and consolidation-path stack; neither
  alone would.
...
\item \emph{Corpus size $N$.}  The \S\ref{sec:flat-failure} collapse
  ...
  disambiguates and heat has nothing to add.  On clean synthetic
  corpora we observe \emph{no} crossover up to $N = 10^5$: the flat
  variant matches or beats the full stack at every $N$ measured
  (see the regime controls below).  The asymptotic argument
  therefore remains empirically untested in our setting; if a
  crossover exists it sits beyond the corpus sizes and
  single-instant ingestion patterns our benchmarks reach.
...
\paragraph{Regime controls.}
Two controls bound the operating regime.  On a uniform-random
synthetic corpus (no access-history structure), \texttt{cortex\_full}
and \texttt{cortex\_flat} produce identical metrics to four decimal
places at $N=100$ (\texttt{benchmarks/results/e2\_zipf/}), confirming
the harness is well-controlled and that decay is inert without
structure.  The full Zipf-$\alpha{=}1.5$ curve
(\texttt{benchmarks/results/e2\_zipf/20260611T015337Z/}) does not
locate a crossover in the full stack's favour---it shows the
opposite: \texttt{cortex\_flat} matches or beats \texttt{cortex\_full}
at every $N$ (MRR full vs flat: $0.980$ vs $0.998$ at $N{=}10^3$,
$0.645$ vs $1.000$ at $10^4$, $0.622$ vs $1.000$ at $10^5$), with
the gap widening as $N$ grows.  The synthetic ground truth may
structurally favour flat relevance retrieval, but the direction
agrees with the clean-store dose--response of
\S\ref{sec:decay-dose}, and we report it accordingly.  Independently,
a longitudinal aging control ($N=5000$, queries bucketed by memory
age 30--360 days; \texttt{benchmarks/results/longitudinal/}) shows
retrieval is stable across age: R@10 ranges 0.98--1.00 from the
youngest to the oldest bucket, i.e.\ decay does not erode
retrievability of old-but-relevant items within a year.
...
retrieval quality, which is also what flat RAG would give them.  As
the store grows and ages, decay's role is protective rather than
discriminative: it keeps accumulated stale content from polluting
retrieval (\S\ref{sec:decay-dose}) while the integrated stack's
ranking lift (\S\ref{sec:empirical}) is carried by fusion, gating,
and consolidation.

\paragraph{The honest framing.}  Decay is not a ranking mechanism on
any corpus we can measure.  Every clean-store measurement we
have---isolated-haystack ablations, the BEAM dose--response, the
Zipf synthetic curve---finds it neutral to slightly negative for
retrieval quality.  Its measured value is operational: it bounds the
effective working set of a long-lived store and suppresses
accumulated stale content, recovering substantial retrieval quality
in a contaminated store ($0.399 \to 0.671$ proxy-MRR,
\S\ref{sec:decay-dose}) while not eroding old-but-relevant items
(the aging control above).  The headline retrieval numbers of
\S\ref{sec:empirical} are carried by the rest of the stack---fusion,
gating, consolidation---not by decay's contribution to ranking.
Whether weeks of genuinely differential live access history would
give decay a ranking role is an open question our single-instant
ingestion benchmarks cannot answer.
...
\subsection{The BEAM Retrieval-Proxy Metric}
\label{sec:beam-proxy}

The BEAM number reported throughout this paper is a
\emph{retrieval-proxy} MRR, defined identically to the sibling
context-assembly paper so the two agree word-for-word.  We measure
the Mean Reciprocal Rank of the first retrieved memory whose content
substring-matches the gold source turn or answer text.  Formally,
for a question $q$ with gold source text $g$, and a ranked list of
retrieved memories $[m_1, m_2, \ldots, m_k]$,
$\text{RR}(q) = 1/\text{rank}(m^*)$ if $\exists\, m^* : g \subseteq
m^*.\text{content}$ and $0$ otherwise, with
$\text{MRR} = \frac{1}{|Q|} \sum_{q \in Q} \text{RR}(q)$.  This is
\emph{not} the BEAM paper's metric.  The paper uses LLM-as-judge
nugget scoring for end-to-end QA quality, which evaluates whether
the reader model's answer contains the key information nuggets.  Our
retrieval-proxy MRR evaluates whether the retrieved context contains
the gold source---a necessary but not sufficient condition for good
answers.  The two are therefore not commensurable, and we make no
head-to-head BEAM claim; the BEAM number is used only for
within-system, same-harness comparisons (e.g.\ the decay
dose--response of \S\ref{sec:decay-dose}), where both conditions are
measured with the identical harness so relative comparisons are
valid.
...
The model's calibration is not workload-independent.  The next
section makes the workload explicit by describing the four
software components that shape what Cortex's memory must be good
at.
...
implementation-independent.  We close by summarising what the
argument and the measurements jointly support, and what they do
not.
...
as both a retrieval prior and a tie-breaker in WSF fusion.  On
three long-term-memory benchmarks, the integrated stack outperforms
the published bests: largest on the question categories that most
require non-flat priors, smaller on surface-fact retrieval where
similarity alone suffices.

The per-mechanism decomposition and the clean-store decay
dose--response (\S\ref{sec:decay-dose}) qualify the attribution: the
ranking lift is carried by fusion, gating, and consolidation, while
heat decay---the mechanism the theoretical argument most directly
motivates---contributes hygiene and capacity control rather than
ranking signal on every corpus we can measure.  The asymptotic
collapse argument (Appendix~\ref{app:shannon}) therefore remains
empirically untested at reachable $N$ and single-instant ingestion;
testing it requires a benchmark whose store accumulates genuinely
differential access history over weeks of live use.  That benchmark,
together with head-to-head re-runs of published baselines on the
same protocol, is the natural next step.
...
(Cortex's WSF operating regime), the resulting distribution is
...
$N \approx 10^4$ (500 questions, $\sim$30 turns/session, $\sim$10k
candidate spans per question); the \emph{aggregate} corpus across
the whole test set is $\approx 10^5$ (used in
\S\ref{app:erdos-lme}).  Using the per-query $N \approx 10^4$ and
plugging into
...
$78.4\%$ paper-best.  The 20.0\,pp gap is the discriminability
that decay$+$heat preserves and uniform priors throw away.

\subsection{WSF Fusion as Multi-Source Decorrelation}
\label{app:shannon-wrrf}

Cortex's \texttt{recall\_memories()} PL/pgSQL function fuses five
signals: vector cosine, FTS (Postgres \texttt{ts\_rank\_cd}),
trigram similarity, heat, and recency.  Treat each signal $s^{(j)}$
as a noisy observation of a hidden relevance target $r_i$:
\[
s_i^{(j)} \;=\; r_i + \varepsilon_i^{(j)},
\qquad j \in \{1, \dots, 5\}.
\]
If the noises $\varepsilon^{(j)}$ are pairwise low-correlation, the
variance of the WSF fused score is
\begin{equation}
\mathrm{Var}\big(\hat r_i\big) \;\approx\;
\frac{\bar\sigma^2}{m \cdot (1 + (m-1)\bar\rho)},
\label{eq:wrrf-var}
\end{equation}
where $m=5$ and $\bar\rho \ll 1$.  With $\bar\rho \approx 0.2$
(rough cross-signal correlation in our ablations), variance shrinks
by ${\sim}3\times$ relative to a single signal.  This effectively
raises $f_S$ in the \S\ref{app:shannon-collapse} formula, which
raises $N^\star$ by the same factor---pushing the collapse
threshold from ${\sim}5$k items to ${\sim}15$k.  This is the
...
\emph{best case} for flat retrieval.  The 20.0\,pp gap in
...
Thermodynamic retrieval mixes both via WSF fusion.
...
\label{eq:thm2-mass}
\end{equation}
\emph{Under WSF fusion of similarity and priority, the head-$k$
...
fraction satisfies the stated bound.  The argument requires only that
the fusion be \emph{monotone non-decreasing} in the priority channel
--- a property shared by reciprocal-rank fusion \citep{Cormack2009}
and by the deployed weighted score fusion \citep{Bruch2023}, since
the priority contribution $w_{\mathrm{heat}} \cdot
\tilde s_{\mathrm{heat}}(x)$ is non-decreasing in heat rank.  For a
head item $i \le k$, its priority rank $\mathrm{rank}_w(i) \le k$
deterministically; the only way it
...
LongMemEval-S \citep{Wu2025} has an aggregate effective corpus
size $N \approx 10^5$ across the whole test set (as distinct from
the $\approx 10^4$ per-query haystack in
\S\ref{app:shannon-collapse}).  Since
...
$\lambda^* \in (0, \infty)$.  Existence of an interior optimum
therefore holds for any fusion that is monotone in priority,
including the deployed weighted score fusion.  \emph{Uniqueness}
is a stronger claim: under the original reciprocal-rank fusion the
per-item score is strictly concave in priority, which makes
$A(\lambda)$ strictly quasiconcave and $\lambda^*$ unique.  The
deployed fusion is affine (linear) in each normalised signal, so
strict concavity does not hold in general; uniqueness of
$\lambda^*$ then requires the additional assumption that
$A(\lambda)$ is single-peaked, which we observe empirically in the
$\lambda$-sweeps (\S\ref{sec:regime}) but do not prove in
closed form.~$\square$

\textbf{Corollary 3.1.} \emph{Cortex's choice of
\texttt{heat\_base} $+$ Ebbinghaus exponential with empirically
tuned half-life is non-arbitrary: there is an interior optimal
decay rate (empirically a single peak), and tuning measures it.}
This justifies treating decay as a measured parameter rather than
...
$\gamma=1.2$, $\sigma=0.05$) demarcates, \emph{within the model},
the regime in which decay is optional (small corpora) from the
regime in which it would become necessary (production-scale
memory).  We caution that this prediction is not corroborated by
...
accumulated over time---are not instantiated by any benchmark we
can currently run; the prediction stands as falsifiable, untested
theory.
...
\begin{itemize}
  ...
  \item Two systems on the same PostgreSQL instance:
        ...
        vector $+$ full-text (FTS) only, constant heat $=$ $0.5$,
        no decay, no WSF heat term.
...
\textbf{Current evidence.} \textbf{TESTED on synthetic; refuted in
that regime.}  The Zipf-$\alpha{=}1.5$ $N$-scan
(\texttt{benchmarks/results/e2\_zipf/20260611T015337Z/};
\S\ref{sec:regime}) runs exactly this protocol on synthetic corpora
and finds the gap \emph{negative and widening in flat's favour}
($N{=}10^3$: full 0.980 vs flat 0.998; $10^4$: 0.645 vs 1.000;
$10^5$: 0.622 vs 1.000).  In the synthetic single-instant-ingestion
regime the monotone-gap prediction is refuted.  The claim survives
only as restricted to corpora with genuinely differential access
history, which remains UNTESTED (each real benchmark provides a
single $N$-point and single-instant ingestion).
...
\texttt{decay\_cycle.py} with constant heat $=$ $0.5$, leaving WSF,
...
\textbf{Current evidence.} \textbf{REFUTED in the measurable
regime.}  The clean-store BEAM dose--response
(\S\ref{sec:decay-dose}) finds decay-off \emph{weakly better} than
decay-on (MRR 0.545 vs 0.503), and the per-mechanism ablations
(\S\ref{sec:per-mechanism}) find decay's isolated contribution
sub-noise-floor to slightly negative on LME-S and LoCoMo.  Decay is
not merely insufficient to prevent collapse on these corpora---it
contributes no ranking signal at all.  The claim could survive only
...
LongMemEval (WSF weights, intent classifier thresholds, FlashRank
...
\textbf{The claim.} ``WSF fusion of 5 signals decorrelates noise
sources.''

\textbf{Riskiest prediction.}  (a)~The $5 \times 5$
...
C4 WSF decorrelates            & medium   & PARTIALLY TESTED & Correlation matrix $+$ ablations \\
...
knows what}.  It changes \emph{who is easy to find}.  WSF---the
...
%----------------------------------------------------------------------
\section{Verification Campaign Provenance}
\label{app:provenance}

This appendix contains the full forensics of the two production
fixes that the \S\ref{sec:per-mechanism} verification campaign
surfaced, relocated here from the main text so the main narrative
states only the summary (\S\ref{sec:per-mechanism}, ``Verification
surfaced two production fixes'').  The integrity of the
\S\ref{sec:per-mechanism} argument requires disclosing every code
path that touched the numbers; this appendix is that disclosure.

\subsection{Verification surfaced a production fix: consolidation cadence}
...
verification improve the system?}  In this instance it did.

\subsection{Verification surfaced a second production fix: plasticity result-shape contract}
\label{sec:plasticity-fix}

A second production-relevant bug surfaced during the same LoCoMo
verification campaign.  The Hebbian update path
(\texttt{apply\_hebbian\_update}) returns a list of result dicts each
carrying an \texttt{action} field that the downstream
\texttt{\_apply\_updates} consumer dispatches on.  The ablation
no-op for plasticity returned raw edge dicts missing the
\texttt{action} key.  Downstream consumed each dict, found no
recognised action, logged a \texttt{WARNING}, and silently dropped
that update from the consolidation pass.  This was a contract bug,
not a crash: rows ran to completion with the plasticity
contribution silently muted on the affected paths.

The fix (commit \texttt{5f737fe}) makes the ablation no-op return
result-shaped dicts with \texttt{action="none"}, restoring contract
...
slightly muted plasticity contribution.  The SYNAPTIC\_PLASTICITY
ablation row is not affected: that row explicitly disables the
plasticity mechanism entirely, so the no-op shape bug cannot
exercise.  The three longitudinal-read-path rows (RECONSOLIDATION,
CO\_ACTIVATION, ADAPTIVE\_DECAY) ran with consolidation off and are
likewise not affected.  The empirical resolution of the
architectural-mismatch hypothesis (\S\ref{sec:locomo-evidence}) does
