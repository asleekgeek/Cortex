...
schema = {
    ...
    "description": (
        ...
        "entities / scoped memory count — a corpus-wide ratio, not a true "
        "per-memory measure), domain balance (distribution of scoped "
        "memories across domains), and compression ratio (penalty for "
        "compressed content). There is no axis scoring which project "
        "source files are remembered. Emits actionable recommendations "
        "(e.g., `run validate_memory`, `run consolidate`). Use this as a "
        "memory-store health check. Distinct from `detect_gaps` (lists "
        "specific missing connections, no aggregate score), "
        "`memory_stats` (raw counts, no scoring), and `narrative`. "
        "Read-only. Latency ~500ms-1s. Returns {coverage_score, "
        "total_memories, age_distribution, entity_density, compression, "
        "domain_balance, recommendations, directory, domain}."
    ),
    "inputSchema": {
        "type": "object",
        "required": [],
